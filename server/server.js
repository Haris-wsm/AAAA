...
app.get("/setstart", async (req, res) => {
  pool.query(
    "SELECT * FROM username_password WHERE user_no = $1",
    [5],
...
app.post("/TestPage", (req, res) => {
  ...
  try {
    ...
    pool.query(
      "SELECT * FROM username_password WHERE username = $1 AND pwd = MD5($2)",
      [username, password],
      function (error, result, fields) {
        ...
        if (result.rows.length) {
          ...
          pool.query(
            "SELECT patient_no FROM patient WHERE user_no = $1",
            [user_no],
            function (error, result, fields) {
...
app.get("/gameone/gettime", (req, res) => {
  pool.query(
    "SELECT a.g1_time FROM g1_difficulty a JOIN patient b " +
...
app.get("/view/weight", (req, res) => {
  ...
  pool.query(
    "SELECT weight FROM patient WHERE user_no = $1",
    [user_no],
    (error, results, fields) => {
      if (error) throw error;
...
app.post("/edit/weight/:weight", (req, res) => {
  ...
  pool.query(
    "UPDATE patient SET weight = $1 WHERE user_no = $2",
    [weight, user_no],
    (error, results, fields) => {
      // console.log(results);
...
app.post("/result/game1", (req, res) => {
  try {
    ...
    pool.query(
      "SELECT create_game_1($1, $2, 1, $3, $4, $5)",
      [score, alert_count, patient_no, set_time, take_time],
...
app.post("/result/game2", (req, res) => {
  try {
    ...
    pool.query(
      "SELECT create_game_2($1, $2, 2, $3, $4, $5, $6)",
      [score, alert_count, patient_no, level_two, success_obj, miss_obj],
      function (error, results, fields) {
        if (error) {
...
app.post("/add_user", (req, res) => {
  ...
  pool.query(
    "INSERT INTO username_password (username, pwd) VALUES ($1, MD5($2))",
...
app.post("/api/access_request", (req, res) => {
  ...
  if (decoded) {
    const query =
      "SELECT a.user_no, a.username, a.pwd, a.department_no, b.department_name " +
    ...
    pool.query(query, [authenSignature], (error, results) => {
      ...
      if (error) {
        ...
        if (results.rows.length) {
          // console.log(results.rows);
          var payload = {
            user_no: results.rows[0].user_no,
            username: results.rows[0].username,
            pwd: results.rows[0].pwd,
            department_no: results.rows[0].department_no,
            department_name: results.rows[0].department_name,
          };
...
app.post("/api/patient/add", checkAuth, async (req, res) => {
  ...
  try {
    ...
    if (!result) {
      ...
      var result1 = await Patient.addPatientUserPass(
        pool,
        input.username,
        input.password
      );
      ...
      if (result1) {
        var result2 = await Patient.selectUser(
          pool,
          input.username,
          input.password
        );
        if (result2) {
          var result3 = await Patient.addPatient(
            pool,
            input.sex,
            input.date,
            input.weight,
          ...
          if (result3) {
            var result4 = await Patient.selectPatientNo(
              pool,
              result2.rows[0].user_no
            );
            if (result4) {
              var result5 = await Patient.addG1(
                pool,
                time,
                result4.rows[0].patient_no
              );
              if (result5) {
...
app.post("/api/patient/update", checkAuth, async (req, res) => {
  ...
  try {
    ...
    var result1 = await Patient.updatePatient(
      pool,
      input.patientNo,
      input.sex,
      input.date,
...
app.post("/api/person/update", checkAuth, async (req, res) => {
  ...
  try {
    await role_user.updatePersonNo(pool, input);
    res.json({
      result: true,
    });
  } catch (ex) {
    ...
    pool.query(query, [authenSignature], (error, results) => {
      ...
      if (error) {
        ...
        if (results.rows.length) {
          console.log("11");
          var payload = {
            user_no: results.user_no,
            username: results.username,
            pwd: results.pwd,
            department_no: results.department_no,
...
app.get("/api/role_user", checkAuth, async (req, res) => {
  let query = "SELECT * FROM role_user ORDER BY name ASC";
